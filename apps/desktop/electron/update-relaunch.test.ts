...
// Lint a script with `bash -n`. On Windows the bare `bash` on PATH may be the
// WindowsApps WSL stub, which cannot resolve the `/mnt/c/...` path (No such
// file or directory / status 127). Invoke `wsl.exe bash` explicitly there so
// the /mnt mount is available; POSIX platforms use the native bash directly.
function lintScript(file: string): void {
  if (process.platform === 'win32') {
    execFileSync('wsl.exe', ['bash', '-n', bashPath(file)], { stdio: 'pipe' })
  } else {
